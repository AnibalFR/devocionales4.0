# 🤖 Workflow de Codex para Cambios y Deployment

Este documento establece el procedimiento estándar que Codex debe seguir al realizar cambios en el proyecto Devocionales 4.0.
...
- **El usuario SIEMPRE inicia Codex en el directorio RAÍZ:** `/Users/anibalfigueroaramirez/XYZ/devocionales4.0/`
- **Codex debe trabajar desde la raíz por defecto** (usar `pwd` para verificar si es necesario)
...
- **DESDE LA RAÍZ (directorio default):** usa `packages/backend/...`, `packages/web/...`, `packages/mobile/...`
  ...
  ```
- **Si Codex se movió a un subdirectorio:** usa paths relativos (`.`, `package.json`, etc.)
  ```bash
...
2. **Crear mensaje de commit descriptivo:**
   ...
   - Incluir automáticamente firma de Codex Code
...
🤖 Generated with [Codex Code](https://claude.com/claude-code)
...
12. ❌ **No verificar directorio actual antes de comandos git** - El usuario inicia Codex en la raíz, pero Codex puede moverse. SIEMPRE usa `pwd` para verificar dónde estás antes de `git add`. Si estás en la raíz, usa `packages/mobile/...`. Si estás en un subdirectorio, usa paths relativos
...
**Contexto:**
- El usuario inicia Codex en: `/Users/anibalfigueroaramirez/XYZ/devocionales4.0/` (RAÍZ)
- Codex puede moverse temporalmente a subdirectorios (ej: para `npm install`)
- Los comandos git deben usar paths correctos según el directorio actual de Codex
...
- **Si estás en la RAÍZ** (lo más común - directorio donde el usuario inicia Codex):
  ```bash
  git add packages/mobile/package.json packages/mobile/package-lock.json
...
- **Si Codex se movió a `packages/mobile`** (raro - solo después de comandos como `cd packages/mobile`):
  ```bash
  git add package.json package-lock.json
  # o todo:
  git add .
...
**❌ ERROR COMÚN:**
Si Codex está en `packages/mobile` y ejecuta `git add packages/mobile/...`, git buscará `packages/mobile/packages/mobile/...` (duplicado) y fallará:
```
...
1. **Agregar archivos modificados:**
   ...
   **Caso Normal - Codex está en la RAÍZ del proyecto (directorio donde el usuario inicia):**
   ```bash
   git add packages/mobile/package.json packages/mobile/package-lock.json
   ```

   **Caso Excepcional - Codex se movió a packages/mobile:**
   ```bash
